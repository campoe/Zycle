...
import com.coenvk.android.zycle.`typealias`.AdapterBlock
...
import com.coenvk.android.zycle.adapter.conditional.ConditionalAdapter
import com.coenvk.android.zycle.adapter.conditional.ConditionalElseAdapter
import com.coenvk.android.zycle.adapter.observer.AdapterDataObservable
import com.coenvk.android.zycle.adapter.observer.AdapterDataObserver
import com.coenvk.android.zycle.adapter.pagination.EndlessScrollAdapter
import com.coenvk.android.zycle.adapter.transformer.Transformer
...
abstract class Adapter : IAdapter {
    ...
    final override fun prepend(adapter: Adapter): Adapter =
        CompositeAdapter(adapter, this)

    final override fun append(adapter: Adapter): Adapter =
        CompositeAdapter(this, adapter)
    ...
    final override fun prepend(layoutRes: Int): Adapter =
        CompositeAdapter(viewAdapterOf(layoutRes), this)

    final override fun prepend(vararg layouts: Int): Adapter {
        return if (layouts.isEmpty()) this
        else CompositeAdapter(viewAdapterOf(*layouts), this)
    }

    final override fun append(layoutRes: Int): Adapter =
        CompositeAdapter(this, viewAdapterOf(layoutRes))

    final override fun append(vararg layouts: Int): Adapter {
        return if (layouts.isEmpty()) this
        else CompositeAdapter(this, viewAdapterOf(*layouts))
    }
    ...
    class Builder private constructor(private var compositeBuilder: CompositeAdapter.Builder) :
        ...
        fun endlessScrollOf(@LayoutRes layoutRes: Int, onLoadNext: (currentPage: Int) -> Unit) =
            append(
